...
HDCallbackCode HDCALLBACK FrictionlessPlaneCallback(void* data)
{
    ...
    case 3: //Virtual Mode
        ...
        case 1: //Coulomb Force
            ...
            break;
        case 2: //Lennard-Jones Potential w/ Exponential Position Scaling
            force[1] = 4 * k[0] * 0.1 * (((12 * pow(4 * k[1] * pow(10, 12), 13)) / (pow(gain * signal + (4.3 * k[1] * pow(10, 12)), 13))) - ((6 * pow(4 * k[1] * pow(10, 12), 7)) / (pow(gain * signal + (4.3 * k[1] * pow(10, 12)), 7))));
...
__declspec(dllexport) void position_reframed_get(double* frame_properties, bool zscaling, double surface_location, double pos_gain, double* output) {   
    ...
    //planing
    if (planing.toggle && num_clicks == 3 && planing.active) {
        if (mode == 2) {
            positionReframed[2] = positionReframed[2] + (planeProperties.d - planeProperties.n[0] * positionReframed[0] - planeProperties.n[1] * positionReframed[1]) / planeProperties.n[2] + 100;
        }
        else {
